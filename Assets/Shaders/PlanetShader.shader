Shader "Custom/PlanetShader"
{
	...
	_ColorEarth("ColorEarth", Color) = (1,1,1,1)
	_ColorMountains("ColorMountains", Color) = (1,1,1,1)
	...
	{
		fixed4 colorWater = tex2D(_MainTex, IN.uv_MainTex) * _ColorWater;
		fixed4 colorEarth = tex2D(_MainTex, IN.uv_MainTex) * _ColorEarth;
		fixed4 colorMountains = tex2D(_MainTex, IN.uv_MainTex) * _ColorMountains;
		
		float height = IN.color.r;
		if (height < 0.45)
		{
			o.Albedo = colorWater.rgb;
			o.Emission = _Emission.xyz;
			o.Alpha = colorWater.a;
		}
		else if (height < 0.75)
		{
			o.Albedo = colorEarth.rgb;
			o.Emission = _Emission.xyz;
			o.Alpha = colorEarth.a;

		}
		else
		{
			o.Albedo = colorMountains.rgb;
			o.Emission = _Emission.xyz;
			o.Alpha = colorMountains.a;
		}
